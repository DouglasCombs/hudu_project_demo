...
const SectionEditProjectStep2 = ({projectId}: {projectId: number}) => {
  ...
  const {handleSubmit, register, setValue, watch, formState} = methods;
  useEffect(() => {
    if (project && addresses) {
      addresses?.map(el => {
        if (
          el?.streetAddress == project?.streetAddress &&
          el?.addressTitle == project?.addressTitle &&
          el?.city == project?.city &&
          el?.state == project?.state
        ) {
          setValue('address', el?.id);
        }
      });
    }
  }, [project, addresses]);
...
export default SectionEditProjectStep2;
